...
export interface Hostel {
  id?: string;
  hostel_name: string;
  hostel_type: "boys" | "girls" | "international" | "staff" | string;
...
const HOSTEL_TYPES = [
  ...
  { value: "international", label: "International Hostel" },
...
export default function AdminHostelsPage() {
  ...
  const [formData, setFormData] = useState<Hostel>(defaultHostel);
  ...
  async function fetchHostelsData() {
    try {
      setIsLoading(true);
      const data = await getHostels();
      const sortedData = (data || []).sort((a: Hostel, b: Hostel) => a.display_order - b.display_order);
      setHostels(sortedData);
    } catch (error) {
      console.error("Failed to load hostels:", error);
      showMessage("error", "Failed to load hostel data.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleCreateNew = () => {
    setFormData({ ...defaultHostel, display_order: hostels.length });
    setView("form");
    setMessage(null);
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (formData.id) {
        const updated = await updateHostel(formData.id, formData);
        setHostels((prev) => prev.map((h) => (h.id === formData.id ? updated : h)).sort((a, b) => a.display_order - b.display_order));
        showMessage("success", "Hostel updated successfully.");
      } else {
        const created = await createHostel(formData);
        setHostels((prev) => [...prev, created].sort((a, b) => a.display_order - b.display_order));
        showMessage("success", "Hostel created successfully.");
      }
      setView("list");
    } catch (error) {
      console.error("Failed to save hostel:", error);
      showMessage("error", "Failed to save hostel data. Please check your inputs.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleReorder = async (index: number, direction: "up" | "down") => {
    if ((direction === "up" && index === 0) || (direction === "down" && index === hostels.length - 1)) return;

    const newHostels = [...hostels];
    const targetIndex = direction === "up" ? index - 1 : index + 1;
    
    const tempOrder = newHostels[index].display_order;
    newHostels[index].display_order = newHostels[targetIndex].display_order;
    newHostels[targetIndex].display_order = tempOrder;

    const temp = newHostels[index];
    newHostels[index] = newHostels[targetIndex];
    newHostels[targetIndex] = temp;

    setHostels(newHostels);

    try {
      if (newHostels[index].id && newHostels[targetIndex].id) {
        await Promise.all([
          updateHostel(newHostels[index].id as string, { display_order: newHostels[index].display_order }),
          updateHostel(newHostels[targetIndex].id as string, { display_order: newHostels[targetIndex].display_order })
        ]);
      }
    } catch (error) {
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1 text-[var(--text-color)] opacity-70">
            Manage institutional residential facilities, accommodations, and warden details.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-black/[0.02] dark:bg-white/[0.02] border-b border-[var(--border-color)] text-[var(--text-color)] opacity-80">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Warden</th>
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
              {hostels.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-[var(--text-color)] opacity-60">
                    No hostels found. Click "Add Hostel" to get started.
                  </td>
                </tr>
              ) : (
                hostels.map((hostel, index) => (
                  <tr key={hostel.id} className="hover:bg-black/[0.01] dark:hover:bg-white/[0.01] transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1 items-center justify-center w-8">
                        ...
                        <span className="text-xs font-mono text-[var(--text-color)]">{hostel.display_order}</span>
                        <button 
                          onClick={() => handleReorder(index, "down")}
                          disabled={index === hostels.length - 1}
                          className="text-[var(--text-color)] opacity-40 hover:opacity-100 disabled:opacity-20 transition-opacity focus:outline-none"
                    ...
                    <td className="px-6 py-4">
                      <span className="font-semibold text-[var(--text-color)]">{hostel.hostel_name}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 text-xs font-medium bg-black/5 dark:bg-white/5 border border-[var(--border-color)] rounded-md text-[var(--text-color)] capitalize">
                        {hostel.hostel_type}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-[var(--text-color)] opacity-90">
                      {hostel.capacity} <span className="opacity-60 text-xs">beds</span>
                    </td>
                    <td className="px-6 py-4 text-[var(--text-color)] opacity-90">
                      {hostel.warden_name || "—"}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <button
                          onClick={() => hostel.id && handleDelete(hostel.id)}
                        ...
                        >
                          {deletingId === hostel.id ? 'Deleting...' : 'Delete'}
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[var(--text-color)]">
              {formData.id ? "Edit Hostel" : "Add New Hostel"}
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Hostel Details</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="hostel_name" className="text-sm font-medium text-[var(--text-color)] opacity-90">Hostel Name *</label>
                <input
                  id="hostel_name" name="hostel_name" required
                  value={formData.hostel_name} onChange={handleChange}
              ...
              <div className="space-y-1.5">
                <label htmlFor="hostel_type" className="text-sm font-medium text-[var(--text-color)] opacity-90">Hostel Category</label>
                <select
                  id="hostel_type" name="hostel_type"
                  value={formData.hostel_type} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none"
                >
              ...
              <div className="space-y-1.5">
                <label htmlFor="capacity" className="text-sm font-medium text-[var(--text-color)] opacity-90">Total Capacity (Beds)</label>
                <input
                  id="capacity" name="capacity" type="number" min="0" required
              ...
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="short_description" className="text-sm font-medium text-[var(--text-color)] opacity-90">Short Summary</label>
                <textarea
                  ...
                  placeholder="Brief introductory text for cards..."
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Content & Amenities</h2>
            
            <div className="space-y-1.5">
              <label htmlFor="description" className="text-sm font-medium text-[var(--text-color)] opacity-90">Detailed Description (HTML)</label>
              <textarea
                ...
                placeholder="<p>Extended description of the hostel infrastructure...</p>"
              />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label htmlFor="facilities" className="text-sm font-medium text-[var(--text-color)] opacity-90">Facilities Available (HTML)</label>
                <textarea
                  id="facilities" name="facilities" rows={4}
                  value={formData.facilities} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none font-mono text-sm resize-y"
                  placeholder="<ul><li>24/7 Wi-Fi</li><li>Mess Facility</li></ul>"
                />
              </div>

              <div className="space-y-1.5">
                <label htmlFor="rules" className="text-sm font-medium text-[var(--text-color)] opacity-90">Rules & Regulations (HTML)</label>
                <textarea
                  id="rules" name="rules" rows={4}
                  value={formData.rules} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none font-mono text-sm resize-y"
                  placeholder="<ul><li>Curfew at 10 PM</li></ul>"
                />
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Administration & Contact</h2>
            
            <div className="space-y-1.5">
              <label htmlFor="warden_name" className="text-sm font-medium text-[var(--text-color)] opacity-90">Warden Name</label>
              <input
                id="warden_name" name="warden_name"
                value={formData.warden_name} onChange={handleChange}
                className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none"
              />
            </div>

            <div className="space-y-1.5">
              <label htmlFor="contact_number" className="text-sm font-medium text-[var(--text-color)] opacity-90">Contact Number</label>
              <input
            ...
            <div className="space-y-1.5">
              <label htmlFor="email" className="text-sm font-medium text-[var(--text-color)] opacity-90">Contact Email</label>
              <input
          ...
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Featured Image</h2>
            ...
            <div className="flex flex-col gap-4">
              {formData.featured_image_url ? (
                <div className="relative aspect-video rounded-lg border border-[var(--border-color)] overflow-hidden group bg-black/5 dark:bg-white/5">
                  <img src={formData.featured_image_url} alt="Hostel Featured" className="w-full h-full object-cover" />
              ...
              <div className="space-y-1.5 pt-2 border-t border-[var(--border-color)]">
                <label htmlFor="featured_image_url" className="text-xs font-medium text-[var(--text-color)] opacity-80">Or enter URL</label>
                <input
          ...
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Configuration & SEO</h2>
