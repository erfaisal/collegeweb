...
const defaultDepartment: HospitalDepartment = {
  department_name: "",
  short_description: "",
  description: "",
  hod_name: "",
  opd_timings: "Mon-Sat: 09:00 AM - 02:00 PM",
  emergency_contact: "",
  featured_image_url: "",
  facilities: "",
  services: "",
...
export default function AdminHospitalPage() {
  ...
  const [formData, setFormData] = useState<HospitalDepartment>(defaultDepartment);
  ...
  async function fetchDepartments() {
    try {
      ...
      const sortedData = (data || []).sort((a: HospitalDepartment, b: HospitalDepartment) => a.display_order - b.display_order);
  ...
  const handleEdit = (dept: HospitalDepartment) => {
    setFormData({ ...dept });
    setView("form");
    setMessage(null);
  };
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this hospital department? This action cannot be undone.")) return;

    try {
      setDeletingId(id);
      await deleteHospitalDepartment(id);
      setDepartments((prev) => prev.filter((d) => d.id !== id));
      showMessage("success", "Hospital department deleted successfully.");
    } catch (error) {
      console.error("Failed to delete hospital department:", error);
      showMessage("error", "Failed to delete hospital department.");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (formData.id) {
        const updated = await updateHospitalDepartment(formData.id, formData);
        setDepartments((prev) => prev.map((d) => (d.id === formData.id ? updated : d)).sort((a, b) => a.display_order - b.display_order));
        showMessage("success", "Hospital department updated successfully.");
      } else {
        const created = await createHospitalDepartment(formData);
        setDepartments((prev) => [...prev, created].sort((a, b) => a.display_order - b.display_order));
        showMessage("success", "Hospital department created successfully.");
      }
      setView("list");
    } catch (error) {
      console.error("Failed to save hospital department:", error);
      showMessage("error", "Failed to save data. Please check your inputs.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleReorder = async (index: number, direction: "up" | "down") => {
    ...
    try {
      if (newDepartments[index].id && newDepartments[targetIndex].id) {
        await Promise.all([
          updateHospitalDepartment(newDepartments[index].id as string, { display_order: newDepartments[index].display_order }),
          updateHospitalDepartment(newDepartments[targetIndex].id as string, { display_order: newDepartments[targetIndex].display_order })
        ]);
      }
    } catch (error) {
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1 text-[var(--text-color)] opacity-70">
            Manage medical departments, OPD timings, and emergency contacts.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-black/[0.02] dark:bg-white/[0.02] border-b border-[var(--border-color)] text-[var(--text-color)] opacity-80">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">HOD / Lead Doctor</th>
              ...
              {departments.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-[var(--text-color)] opacity-60">
                    No hospital departments found. Click "Add Department" to get started.
                  </td>
                </tr>
              ) : (
                departments.map((dept, index) => (
                  <tr key={dept.id} className="hover:bg-black/[0.01] dark:hover:bg-white/[0.01] transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-semibold text-[var(--text-color)]">{dept.department_name}</span>
                    </td>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[var(--text-color)]">
              {formData.id ? "Edit Hospital Department" : "Add New Department"}
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Department Details</h2>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="department_name" className="text-sm font-medium text-[var(--text-color)] opacity-90">Department Name *</label>
                <input
                  id="department_name" name="department_name" required
                  value={formData.department_name} onChange={handleChange}
              ...
              <div className="space-y-1.5">
                <label htmlFor="hod_name" className="text-sm font-medium text-[var(--text-color)] opacity-90">HOD / Lead Doctor</label>
                <input
              ...
              <div className="space-y-1.5">
                <label htmlFor="emergency_contact" className="text-sm font-medium text-[var(--text-color)] opacity-90">Emergency Contact</label>
                <input
                  id="emergency_contact" name="emergency_contact" type="tel"
              ...
              <div className="space-y-1.5 sm:col-span-2">
                <label htmlFor="opd_timings" className="text-sm font-medium text-[var(--text-color)] opacity-90">OPD Timings</label>
                <input
                  id="opd_timings" name="opd_timings"
                  value={formData.opd_timings} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none"
          ...
          <div className="bg-[var(--background-color)] border border-[var(--border-color)] rounded-xl p-5 sm:p-6 space-y-5 shadow-sm">
            <h2 className="text-lg font-semibold text-[var(--text-color)] mb-4">Content & Services</h2>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="<p>Extended description of the medical department...</p>"
              />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label htmlFor="services" className="text-sm font-medium text-[var(--text-color)] opacity-90">Key Services (HTML)</label>
                <textarea
                  id="services" name="services" rows={4}
                  value={formData.services} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none font-mono text-sm resize-y"
                  placeholder="<ul><li>Cardiac Surgery</li><li>Echocardiography</li></ul>"
                />
              </div>

              <div className="space-y-1.5">
                <label htmlFor="facilities" className="text-sm font-medium text-[var(--text-color)] opacity-90">Facilities & Equipment (HTML)</label>
                <textarea
                  id="facilities" name="facilities" rows={4}
                  value={formData.facilities} onChange={handleChange}
                  className="w-full px-3 py-2 bg-[var(--background-color)] border border-[var(--border-color)] rounded-lg text-[var(--text-color)] focus:ring-2 focus:ring-indigo-500/50 outline-none font-mono text-sm resize-y"
                  placeholder="<ul><li>Advanced Cath Lab</li></ul>"
                />
              </div>
            </div>
          </div>
